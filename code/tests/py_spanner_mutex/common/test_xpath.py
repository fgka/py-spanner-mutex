...
from py_spanner_mutex.common import xpath
...
def test_remove_keys_based_on_paths_ok_mixed():
    # Given
    value = {
        "root": {
            "node_a": [1, 2, 3],
            "entry_lst": [
                {
                    "key_a": [
                        _create_object_from_value(dict(node_a=dict(value=123), node_b="asdf")),
                        _create_object_from_value(dict(node_a=dict(value=321), node_b="qwer")),
                    ]
                },
            ],
        }
    ...
    paths = [
        "root.node_a",
        "non_node",
        "root.entry_lst[].key_a[].node_b",
        "root.entry_lst[].key_a[].node_c",
        "root.entry_lst[].non_key[].non_node",
    ]
    expected = {
        "root": {
            "entry_lst": [
                {
                    "key_a": [
                        _create_object_from_value(dict(node_a=dict(value=123))),
                        _create_object_from_value(dict(node_a=dict(value=321))),
                    ]
                },
            ]
        }
    ...
    # When
    result = xpath.remove_keys_based_on_paths(value, paths)
    # Then
    assert result == value
